...
import ProjectCard from './dashboard/ProjectDashboard';
...
const GroupPage = () => {
  ...
  const [projects, setProjects] = useState([]); // New state to store projects
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      try {
        ...
        const projectQuery = query(collection(firestore, 'projects'), where('groupId', '==', groupId));
        ...
        const taskQuery = query(collection(firestore, 'tasks'), where('groupId', '==', groupId));
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      ...
      // After the user is removed, navigate to the home page or any other page
      navigate('/');
    } catch (err) {
      console.error('Error leaving group:', err);
      alert('Error leaving group. Please try again later.');
    }
  };
  
  const handleGenerateInviteCode = async () => {
    const generateCode = () => Math.random().toString(36).substring(2, 10); // Generate a random string
    let uniqueCode = generateCode(); // Generate initial code

    const checkIfCodeExists = async (code) => {
      const groupsQuery = query(collection(firestore, 'groups'), where('inviteCode', '==', code));
      const groupsSnapshot = await getDocs(groupsQuery);
      return !groupsSnapshot.empty; // If the snapshot is not empty, the code exists
    };
    ...
    try {
      ...
      while (!isUnique) {
        ...
        if (!isUnique) {
          uniqueCode = generateCode(); // Generate a new code if the previous one was not unique
        }
      }

      // Now that we have a unique invite code, update the group document
      await updateDoc(doc(firestore, 'groups', groupId), {
        inviteCode: uniqueCode,
      });
      setInviteCode(uniqueCode); // Update the invite code in state
    } catch (err) {
