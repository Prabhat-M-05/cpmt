...
const MyProjects = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(firestore, 'projects'), async (snapshot) => {
      const now = new Date();
      ...
      const dataWithPoints = await Promise.all(
        snapshot.docs.map(async (docSnap) => {
          ...
          const subRef = collection(firestore, `projects/${docSnap.id}/completedPoints`);
  ...
  const handleStartPause = async (project) => {
    const ref = doc(firestore, 'projects', project.id);
    const now = new Date();
  ...
  const handleLogStoryPoints = async () => {
    ...
    const subRef = collection(firestore, `projects/${selectedProject.id}/completedPoints`);
    await addDoc(subRef, {
      points,
      timestamp: new Date(),
    });
  ...
  const fetchBurndownData = async (projectId, totalPoints) => {
    const subRef = collection(firestore, `projects/${projectId}/completedPoints`);
